...
describe('salesReportQuerySchema', () => {
  it('from／to 都必須是 YYYYMMDD 格式', () => {
    expect(salesReportQuerySchema.safeParse({ from: '20260101', to: '20260107' }).success).toBe(
      true
    )
    expect(salesReportQuerySchema.safeParse({ from: '2026-01-01', to: '20260107' }).success).toBe(
      false
    )
  })

  it('from 不能晚於 to', () => {
    expect(salesReportQuerySchema.safeParse({ from: '20260107', to: '20260101' }).success).toBe(
      false
    )
    expect(salesReportQuerySchema.safeParse({ from: '20260101', to: '20260101' }).success).toBe(
      true
    )
  })
})
...
describe('salesReportSchema', () => {
  it('接受完整報表回應', () => {
    const result = salesReportSchema.safeParse({
      ...
      refundAmount: 0,
      channelBreakdown: [
        { channel: '內用', count: 5, revenue: 2000 },
        { channel: '外帶', count: 4, revenue: 1400 }
      ],
