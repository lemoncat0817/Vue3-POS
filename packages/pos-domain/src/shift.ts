 ...
 * 收班實際點鈔金額，算出「應有現金」與「帳差」。跟計價一樣，這個
 * 計算本身值得單獨測試、不依賴資料庫或 HTTP，實際的班別開關、現金
 * 營業額查詢留給 apps/api/src/routes/shifts.ts。
 */
...
export function summarizeShiftCash(input: ShiftCashSummaryInput): ShiftCashSummary {
  const expectedCash = input.openingFloat + input.cashSales + input.cashIn - input.cashOut
  return {
    expectedCash,
