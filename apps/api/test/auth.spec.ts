...
async function createStaff(db: AnyDb, app: Awaited<ReturnType<typeof createTestAppWithDevice>>['app'], deviceToken: string, staffId: string) {
...
describe('POST /api/auth/operator-login', () => {
  ...
  it('帳號與 PIN 都正確時登入成功，回傳員工資料（不含 PIN）', async () => {
    ...
    await createStaff(db, app, deviceToken, staffId)
