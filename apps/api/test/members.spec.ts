...
describe('GET /api/members', () => {
  it('可以用手機號碼查詢，結帳當下用得到（不需要拉全部會員清單）', async () => {
    ...
    const res = await app.request('/api/members?phone=0987654321', { headers: { 'X-Device-Token': deviceToken, 'X-Operator-Session': sessionToken } })
    const body = await readJson(res)
...
describe('GET /api/members/:id、PUT、DELETE', () => {
  ...
  it('更新資料、刪除都正常運作；改成別人已經用過的手機號碼會被擋（409）', async () => {
    ...
    const deleteRes = await app.request(`/api/members/${a.id}`, { method: 'DELETE', headers: { 'X-Device-Token': deviceToken, 'X-Operator-Session': sessionToken } })
    expect(deleteRes.status).toBe(204)
    expect((await app.request(`/api/members/${a.id}`, { headers: { 'X-Device-Token': deviceToken, 'X-Operator-Session': sessionToken } })).status).toBe(404)
  })
  ...
  it('刪除有消費紀錄的會員：訂單本身保留，只是解除會員關聯，不是被 FK 約束擋下來', async () => {
    ...
    const deleteRes = await app.request(`/api/members/${member.id}`, { method: 'DELETE', headers: { 'X-Device-Token': deviceToken, 'X-Operator-Session': sessionToken } })
    expect(deleteRes.status).toBe(204)
