...
describe('POST /api/shifts', () => {
  ...
  it('已經有一筆班別開帳中時，再開新的班別拒絕，回傳 409', async () => {
    ...
    await app.request('/api/shifts', {
      ...
      body: JSON.stringify({
        shiftId: '01ARZ3NDEKTSV4RRFFQ69G5FA1',
        operator: '店長 - Lemon',
        openingFloat: 3000
      })
    })
    const res = await app.request('/api/shifts', {
      ...
      body: JSON.stringify({
        shiftId: '01ARZ3NDEKTSV4RRFFQ69G5FA2',
        operator: '值班經理',
        openingFloat: 3000
      })
    })
...
describe('POST /api/shifts/:id/close', () => {
  it('收班時把班別期間的現金訂單算進 cashSales，非現金訂單不計入，帳差為 0（點鈔剛好對得起來）', async () => {
    ...
    await app.request('/api/orders', {
      ...
      body: JSON.stringify(
        buildOrderRequest('01ARZ3NDEKTSV4RRFFQ69G5FB1', [{ method: '現金', amount: 80 }])
      )
    })
    await app.request('/api/orders', {
      method: 'POST',
      ...
      body: JSON.stringify(
        buildOrderRequest('01ARZ3NDEKTSV4RRFFQ69G5FB2', [{ method: '信用卡', amount: 80 }])
      )
    })
