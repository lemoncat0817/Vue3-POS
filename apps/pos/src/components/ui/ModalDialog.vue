      ...
      <DialogContent
        ...
        <div class="flex shrink-0 items-center justify-between px-6 pt-6">
          <div>
            ...
            <DialogDescription v-if="description" class="mt-1 text-xs text-surface-500 dark:text-surface-400">
              {{ description }}
            </DialogDescription>
...
const props = withDefaults(defineProps<{ open: boolean; title: string; description?: string; size?: 'sm' | 'md' | 'lg' | 'xl' }>(), {
  description: '',
  size: 'md',
})
