  ...
  <DialogRoot :open="state.open" @update:open="onOpenChange">
    <DialogPortal>
      ...
      <DialogContent
        ...
        <DialogTitle class="text-lg font-bold text-surface-900 dark:text-surface-100">退款</DialogTitle>
        <DialogDescription class="mt-2 text-sm text-surface-600 dark:text-surface-400">
          這筆訂單目前還能退 <span class="font-bold text-primary-600 dark:text-primary-400">$ {{ state.max }}</span>
        </DialogDescription>
...
const isValid = computed(() => state.amount > 0 && state.amount <= state.max && state.reason.trim() !== '')
...
function onOpenChange(open: boolean) {
  if (!open) {
    settleRefundPrompt(pendingResult.value === 'confirm' && isValid.value ? { amount: state.amount, reason: state.reason.trim() } : null)
