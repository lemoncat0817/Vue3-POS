<template>
  <div class="flex flex-col gap-1.5">
    ...
    >
      ...
      <SelectPortal>
        <SelectContent
          class="z-50 min-w-[220px] rounded-xl border border-surface-200 dark:border-surface-800 bg-white dark:bg-surface-900 p-1 shadow-lg"
          position="popper"
        >
          <SelectViewport class="p-1">
            <SelectItem
              v-for="role in rolesStore.roleList"
              :key="role.id"
              :value="role.id"
            ...
            >
              <SelectItemText>
                {{ role.name }}
                <span class="font-mono font-normal text-surface-400"
                  >{{ role.capabilities.length }}/{{ AUTHORITY_FIELDS.length }}</span
                >
              </SelectItemText>
            </SelectItem>
          </SelectViewport>
