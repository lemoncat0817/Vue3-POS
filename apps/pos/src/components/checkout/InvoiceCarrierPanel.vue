<template>
  <button
    ...
    data-testid="invoice-carrier-button" @click="openDialog">
    {{ triggerLabel }}
  </button>

  <ModalDialog :open="open" title="發票載具" @update:open="onOpenChange">
    <div class="flex flex-col gap-3">
      <p class="text-xs text-surface-500 dark:text-surface-400">選擇這張發票怎麼開立，預設是無載具（紙本發票）。</p>
      <div class="flex gap-2">
        <button
v-for="option in typeOptions" :key="option"
          type="button"
          class="flex-1 rounded-lg px-3 py-2 text-sm font-bold transition-colors"
          :class="draftType === option
      ...
      <label v-if="draftType !== '無載具'" class="block text-xs font-bold text-surface-500 dark:text-surface-400">
        {{ draftType === '手機條碼' ? '手機條碼（「/」開頭＋7碼）' : '統一編號（8碼數字）' }}
        <input
v-model="draftValue" type="text"
      ...
      <p v-if="errorMessage" class="text-xs font-bold text-danger-600 dark:text-danger-400">{{ errorMessage }}</p>
...
type="button" :disabled="!isValid"
          ...
          data-testid="confirm-invoice-carrier" @click="confirm">
          確定
        </button>
      </div>
    </div>
...
const draft = computed<InvoiceCarrier>(() =>
  draftType.value === '無載具' ? { type: '無載具' } : { type: draftType.value, value: draftValue.value },
)
