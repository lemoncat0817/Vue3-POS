<template>
  <div class="w-full flex flex-col p-4">
    ...
    <div v-if="activeTab === 'orderCoupons'" class="flex flex-col">
      <div class="flex items-center justify-between pb-3 mb-3 border-b border-surface-100 dark:border-surface-800">
        <div>
          <span class="text-sm font-black text-surface-900 dark:text-surface-100">
            訂單折價券<span class="ml-2 rounded-full bg-surface-100 dark:bg-surface-800 px-2 py-0.5 text-[10px] font-bold text-surface-600 dark:text-surface-300">共 {{ discountStore.orderCoupons.length }} 樣</span>
          </span>
          <p class="mt-0.5 text-[11px] text-surface-400">結帳時整張訂單套用一張，例如「$50折價券」「整單95折」</p>
        </div>
        <button
          type="button" class="pos-btn pos-btn-primary px-3 py-1.5 text-xs"
          :class="{ 'opacity-50 pointer-events-none': !canSetOrderCoupon }"
          @click="openAddOrderCouponDialog">＋ 新增折價券</button>
      </div>
        ...
        <table class="w-full text-center text-sm">
          ...
          <tbody class="divide-y divide-surface-100 dark:divide-surface-800">
            <tr v-if="sliceOrderCoupons.length === 0">
              <td colspan="4" class="px-2 py-8 text-surface-400 dark:text-surface-500">無折價券</td>
            </tr>
            <tr v-for="row in sliceOrderCoupons" :key="row.id" class="hover:bg-surface-50 dark:hover:bg-surface-950/40">
              ...
              <td class="px-3 py-2.5 text-right font-mono font-bold" :class="row.kind === 'amount' ? 'text-success-600 dark:text-success-400' : 'text-info-600 dark:text-info-400'">
                {{ row.kind === 'amount' ? `-$${row.value}` : row.value }}
              </td>
              <td class="px-3 py-2.5">
                <div class="flex items-center justify-center gap-1.5">
                  <button type="button" class="pos-btn pos-btn-secondary px-2.5 py-1 text-xs" :class="{ 'opacity-50 pointer-events-none': !canSetOrderCoupon }" @click="openEditOrderCouponDialog(row)">編輯</button>
                  <button type="button" class="pos-btn pos-btn-danger px-2.5 py-1 text-xs" :class="{ 'opacity-50 pointer-events-none': !canSetOrderCoupon }" @click="removeOrderCoupon(row)">刪除</button>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <div class="mt-3 flex items-center justify-between rounded-xl bg-surface-50 dark:bg-surface-800/60 px-3 py-2 text-xs font-bold text-surface-600 dark:text-surface-300">
        ...
        <AppPagination :page="orderCouponPage" :page-count="orderCouponPageCount" :total="discountStore.orderCoupons.length" @update:page="(v) => orderCouponPage = v" />
      </div>
    </div>
    ...
    <div v-if="activeTab === 'quickDiscounts'" class="flex flex-col">
      <div class="flex items-center justify-between pb-3 mb-3 border-b border-surface-100 dark:border-surface-800">
        <div>
          <span class="text-sm font-black text-surface-900 dark:text-surface-100">
            快速折扣<span class="ml-2 rounded-full bg-surface-100 dark:bg-surface-800 px-2 py-0.5 text-[10px] font-bold text-surface-600 dark:text-surface-300">共 {{ discountStore.quickDiscounts.length }} 樣</span>
          </span>
          <p class="mt-0.5 text-[11px] text-surface-400">點餐頁購物車勾選品項後可直接套用的快捷折扣按鈕</p>
        </div>
        <button
          type="button" class="pos-btn pos-btn-primary px-3 py-1.5 text-xs"
        ...
        <table class="w-full text-center text-sm">
          ...
          <tbody class="divide-y divide-surface-100 dark:divide-surface-800">
            <tr v-if="sliceQuickDiscount.length === 0">
              <td colspan="4" class="px-2 py-8 text-surface-400 dark:text-surface-500">無快速折扣</td>
            </tr>
            <tr v-for="row in sliceQuickDiscount" :key="row.id" class="hover:bg-surface-50 dark:hover:bg-surface-950/40">
              ...
              <td class="px-3 py-2.5">
                <div class="flex items-center justify-center gap-1.5">
                  <button type="button" class="pos-btn pos-btn-secondary px-2.5 py-1 text-xs" :class="{ 'opacity-50 pointer-events-none': !canSetQuickDiscount }" @click="openEditQuickDiscountDialog(row)">編輯</button>
                  <button type="button" class="pos-btn pos-btn-danger px-2.5 py-1 text-xs" :class="{ 'opacity-50 pointer-events-none': !canSetQuickDiscount }" @click="removeQuickDiscount(row)">刪除</button>
                </div>
              </td>
            </tr>
          </tbody>
  ...
  <ModalDialog v-model:open="addOrderCouponDialog" title="新增訂單折價券">
    <Form v-slot="{ isSubmitting, values, setFieldValue }" :validation-schema="toTypedSchema(orderCouponSchema())" :initial-values="{ name: '', kind: 'amount', value: 0 }" @submit="onSubmitAddOrderCoupon">
      <div class="space-y-4 py-2">
        <FormField name="name" label="折價券名稱" :disabled="isSubmitting" placeholder="例如: $50折價券、整單95折..." />
        <label class="mb-4 block text-sm font-bold text-surface-700 dark:text-surface-300">
          類型
          <select :value="values.kind" class="mt-1 w-full rounded-lg border border-surface-300 dark:border-surface-700 bg-white dark:bg-surface-800 px-3 py-2 text-sm font-normal text-surface-900 dark:text-surface-100 outline-none" @change="setFieldValue('kind', ($event.target as HTMLSelectElement).value)">
            <option value="amount">定額折抵</option>
            <option value="percent">折數折抵</option>
          </select>
        </label>
        <FormField
          name="value" :label="values.kind === 'amount' ? '折抵金額' : '折抵折數'"
          type="number" :step="values.kind === 'amount' ? '1' : '0.01'" :disabled="isSubmitting"
          :placeholder="values.kind === 'amount' ? '純數字,例如:50,100...' : '純數字,例如:0.9,0.75...'" />
      </div>
      <div class="mt-6 flex justify-end gap-2.5">
        <button type="button" class="pos-btn pos-btn-secondary px-4 py-2 text-xs font-bold" @click="addOrderCouponDialog = false">取消</button>
  ...
  <ModalDialog v-model:open="editOrderCouponDialog" title="編輯訂單折價券">
    <Form
      ...
      :initial-values="{ name: editingOrderCoupon.name, kind: editingOrderCoupon.kind, value: Number(editingOrderCoupon.value) }"
      ...
      <div class="mt-6 flex justify-end gap-2.5">
        <button type="button" class="pos-btn pos-btn-secondary px-4 py-2 text-xs font-bold" @click="editOrderCouponDialog = false">取消</button>
  ...
  <ModalDialog v-model:open="addQuickDiscountDialog" title="新增快速折扣">
    <Form v-slot="{ isSubmitting, values, setFieldValue }" :validation-schema="toTypedSchema(quickDiscountSchema())" :initial-values="{ name: '', kind: 'amount', value: 0 }" @submit="onSubmitAddQuickDiscount">
      <div class="space-y-4 py-2">
        ...
        <FormField
          name="value" :label="values.kind === 'amount' ? '折抵金額' : '折抵折數'"
          type="number" :step="values.kind === 'amount' ? '1' : '0.01'" :disabled="isSubmitting"
          :placeholder="values.kind === 'amount' ? '純數字,例如:5,10...' : '純數字,例如:0.9,0.85...'" />
      </div>
      <div class="mt-6 flex justify-end gap-2.5">
  ...
  <ModalDialog v-model:open="editQuickDiscountDialog" title="編輯快速折扣">
    <Form
      ...
      :initial-values="{ name: editingQuickDiscount.name, kind: editingQuickDiscount.kind, value: Number(editingQuickDiscount.value) }"
...
const orderCouponPageCount = computed(() => Math.max(Math.ceil(discountStore.orderCoupons.length / 10), 1))
...
function orderCouponSchema(excludeId?: OrderCoupon['id']) {
  return z.object({
    name: z.string().trim().min(1, '請輸入折價券名稱').refine(
      (name) => !discountStore.orderCoupons.some((item) => item.name === name && item.id !== excludeId),
      '此折價券名稱已存在,請重新輸入',
    ),
...
const sliceQuickDiscount = computed(() =>
  discountStore.quickDiscounts.slice((quickDiscountPage.value - 1) * 10, quickDiscountPage.value * 10))
