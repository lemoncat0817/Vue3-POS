  ...
  <div class="w-full flex flex-col p-4">
    <div class="flex items-center justify-between pb-3 mb-3 border-b border-surface-100 dark:border-surface-800">
      <span class="text-sm font-black text-surface-900 dark:text-surface-100">
        人員名單<span class="ml-2 rounded-full bg-surface-100 dark:bg-surface-800 px-2 py-0.5 text-[10px] font-bold text-surface-600 dark:text-surface-300">共 {{ authorityManagementStore.staffList.length }} 位</span>
      </span>
      <button
        type="button" class="pos-btn pos-btn-primary px-3 py-1.5 text-xs"
        :class="{ 'opacity-50 pointer-events-none': !canManage }"
        @click="openAddStaffDialog">＋ 新增人員</button>
    </div>
    ...
    <div class="overflow-hidden rounded-xl border border-surface-200 dark:border-surface-800">
      <table class="w-full text-center text-sm">
        ...
        <tbody class="divide-y divide-surface-100 dark:divide-surface-800">
          ...
          <tr v-for="row in sliceStaffList" :key="row.id" class="hover:bg-surface-50 dark:hover:bg-surface-950/40">
            ...
            <td class="px-3 py-2.5">
              <div class="flex items-center justify-center gap-1.5">
                <button
                  ...
                  @click="openEditStaffDialog(row)">編輯</button>
                <button
                  type="button" class="pos-btn pos-btn-danger px-2.5 py-1 text-xs"
                  :class="{ 'opacity-50 pointer-events-none': !canEdit(row) }"
                  @click="deleteStaff(row)">刪除</button>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <div class="mt-3 flex items-center justify-between rounded-xl bg-surface-50 dark:bg-surface-800/60 px-3 py-2 text-xs font-bold text-surface-600 dark:text-surface-300">
      <p>{{ `共 ${authorityManagementStore.staffList.length} 位` }}</p>
    ...
    <ModalDialog v-model:open="addStaffDialog" title="新增人員" size="lg">
      <div class="space-y-3.5 py-1">
        ...
        <div>
          <label class="mb-1.5 block text-xs font-bold text-surface-700 dark:text-surface-300">權限群組</label>
...
async function addStaff() {
  if (currentInputStaffName.value === '' || currentInputStaffJobTitle.value === '' || currentInputStaffAccount.value === '' || currentInputStaffPin.value === '' || currentInputRoleId.value === '') {
...
async function editStaff() {
  ...
  if (!target) return
  if (currentEditInputStaffName.value === '' || currentEditInputStaffJobTitle.value === '' || currentEditInputStaffAccount.value === '' || currentEditRoleId.value === '') {
    showToast('請輸入完整資訊', 'error')
    return
  }
  ...
  if (
    currentEditInputStaffName.value === target.name && currentEditInputStaffJobTitle.value === target.jobTitle
    && currentEditInputStaffAccount.value === target.account && currentEditInputStaffPin.value === '' && currentEditRoleId.value === target.roleId
  ) {
    editStaffDialog.value = false
