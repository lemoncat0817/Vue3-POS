  ...
  <div class="w-full flex flex-col">
    ...
    >
        ...
        <table class="w-full text-left text-sm">
          ...
          >
            <tr>
              ...
              <th class="px-4 py-3.5 text-center">權限內容</th>
              <th class="px-4 py-3.5 text-center">使用人數</th>
            ...
            <tr v-if="sliceRoleList.length === 0">
              <td colspan="5" class="px-4 py-16 text-center text-surface-400 dark:text-surface-500">
                <div class="flex flex-col items-center justify-center gap-2">
                  <Shield class="h-10 w-10 text-surface-300 dark:text-surface-700" />
                  <span class="text-base font-semibold text-surface-700 dark:text-surface-300"
                    >還沒有任何權限群組</span
                  >
                  <span class="text-xs text-surface-400 dark:text-surface-500"
                    >尚未建立權限群組資料，可點選上方「＋ 新增權限群組」</span
                  >
                </div>
              </td>
            </tr>
            <tr
              v-for="row in sliceRoleList"
              :key="row.id"
            ...
            >
              ...
              <td class="px-4 py-3.5 align-middle text-center">
                <span
                  ...
                  :class="
                    row.isSystem
                      ? 'bg-accent-50 text-accent-600 dark:bg-accent-950/50 dark:text-accent-400 border border-accent-200 dark:border-accent-800'
      ...
      <TablePagination
        ...
        :total="rolesStore.roleList.length"
        :current-count="sliceRoleList.length"
        unit="組權限群組"
        @update:page="handleRoleCurrentChange"
      />
    </div>

    <ModalDialog
      v-model:open="dialog.open"
      :title="dialog.editingId ? '編輯權限群組' : '新增權限群組'"
      size="lg"
    >
      <div class="space-y-3.5 py-1">
        <div>
          ...
          />
          <p v-if="dialog.isSystem" class="mt-1 text-[11px] text-surface-400">
            系統內建角色不可改名，但權限內容仍可調整。
          </p>
        </div>
...
async function submit() {
  ...
  if (
    rolesStore.roleList.some((item) => item.name === dialog.name && item.id !== dialog.editingId)
  ) {
  ...
  try {
    if (dialog.editingId === null) {
      ...
      authorityManagementStore.staffList = authorityManagementStore.staffList.map((staff) =>
        staff.roleId === updated.id
          ? { ...staff, roleName: updated.name, authorityCheckList: updated.capabilities }
          : staff
      )
