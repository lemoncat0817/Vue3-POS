<template>
  <div class="w-full h-full flex flex-col ">
    ...
    <div class="w-full h-full grid grid-cols-5 place-items-center">
      <div
v-for="item in sliceDrinkMenu" :key="item.id" class="2xl:w-28 2xl:h-28 xl:w-24 xl:h-24 lg:w-[72px] lg:h-[72px] md:w-14 md:h-14 sm:w-12 sm:h-12 w-11 h-11 bg-red-400 border-solid border-2 border-black rounded-lg  cursor-pointer flex justify-center items-center"
        :class="{ 'bg-yellow-400': item.name === fromSelection(drinkStore.drinkItem)?.name }"
