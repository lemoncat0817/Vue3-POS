  ...
  <div class="w-full flex flex-col">
    <div class="flex items-center justify-between mb-2 px-1">
      <div class="flex items-center gap-2">
        <span class="text-xs font-black uppercase tracking-wider text-surface-500 dark:text-surface-400">品項選單</span>
      ...
      <AppPagination v-if="pageCount > 1" :page="currentPage" :page-count="pageCount" :total="currentProducts.length" @update:page="handleCurrentChange" />
    </div>

    <div v-if="currentProducts.length === 0" class="flex flex-col items-center justify-center py-12 rounded-2xl border border-dashed border-surface-300 dark:border-surface-700 text-surface-400">
    ...
    <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2.5">
      <button
        v-for="item in sliceProducts" :key="item.id"
        type="button"
        ...
        :class="{
          'border-primary-500 bg-primary-50/90 dark:bg-primary-950/40 text-primary-900 dark:text-primary-100 ring-2 ring-primary-500/30 scale-[1.02]': item.name === fromSelection(catalogStore.selectedProduct)?.name,
          'border-surface-200 dark:border-surface-700/80 bg-white dark:bg-surface-800 text-surface-800 dark:text-surface-100 hover:border-surface-300 dark:hover:border-surface-600': item.name !== fromSelection(catalogStore.selectedProduct)?.name,
...
const currentProducts = computed(() => {
  ...
  return catalogStore.products.filter((product) => String(product.categoryId) === catalogStore.selectedCategoryId)
})
