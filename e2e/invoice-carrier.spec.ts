...
test('選擇手機條碼載具後送單，伺服端記錄的載具與發票號碼都正確，畫面上看得到', async ({ page }) => {
  ...
  const detailPanel = row.locator('xpath=following-sibling::tr[1]')
  await expect(detailPanel).toContainText(`發票 ${createBody.invoiceNumber}`)
  await expect(detailPanel).toContainText('手機條碼 /ABC1234')
})
