<template>
  <!-- 飲品類型 -->
  <div class="xl:flex-[1] w-[30%]">
    <div class="flex justify-between mt-2">
      <div class="ml-2 lg:text-lg text-sm text-blue-800 font-bold border-b-2 border-solid border-black">飲品類型</div>
      <div class="flex mr-2">
        <!-- 新增功能 -->
        <button @click="openAddTypeDialog"
          class="lg:px-2 px-0.5 border-2 border-solid border-black rounded-lg mx-1 lg:text-md text-sm text-blue-800 font-bold bg-red-500 select-none active:bg-yellow-300"
        ...
        <button @click="deleteDrinkType"
          :class="{ 'opacity-50': loginStore.userInfo.canSetDrinkType === 'X', 'pointer-events-none': loginStore.userInfo.canSetDrinkType === 'X' }"
  ...
  <!-- 飲料品項 -->
  <div class="xl:flex-[1] w-[40%] border-x-2 border-solid border-black rounded-lg">
    <div class="flex justify-between mt-2">
      <div class="ml-2 lg:text-lg text-sm text-blue-800 font-bold border-b-2 border-solid border-black">飲料品項</div>
      <div class="flex mr-2">
        <!-- 新增功能 -->
        <button @click="openAddDrinkDialog"
          :class="{ 'opacity-50': loginStore.userInfo.canSetDrink === 'X', 'pointer-events-none': loginStore.userInfo.canSetDrink === 'X' }"
        ...
        <el-dialog v-model="editDrinkDialog" title="編輯飲料品項" width="500">
          ...
          <div class="w-4/5 flex justify-between items-center text-blue-800 text-lg font-bold my-2">
            ...
            <div class="flex">
              ...
              <input v-if="setEditPriceBottle" v-model="currentEditDrinkInputPriceBottle" type="number" min="1" step="1"
                class="border-2 border-solid border-black rounded-lg ml-2 text-center px-2"
                placeholder="純數字,例如:1,2,3..." />
              <div v-else class="w-[235px] border-2 border-solid border-black rounded-lg ml-2 text-center px-2">
                none
    ...
    <div>
      <el-table class="cursor-pointer mt-2" :data="sliceDrink" highlight-current-row height="440" empty-text="請先選擇飲品類型"
  ...
  <div class="xl:flex-[1] w-[30%]">
    <div class="flex justify-between mt-2">
      <div class="ml-2 lg:text-lg text-sm text-blue-800 font-bold border-b-2 border-solid border-black">配料</div>
      <div class="flex mr-2">
        <!-- 新增功能 -->
        <button @click="openAddIngredientsDialog"
          :class="{ 'opacity-50': loginStore.userInfo.canSetIngredients === 'X', 'pointer-events-none': loginStore.userInfo.canSetIngredients === 'X' }"
          class="lg:px-2 px-0.5 border-2 border-solid border-black rounded-lg mx-1 lg:text-md text-sm text-blue-800 font-bold bg-red-500 select-none active:bg-yellow-300">新增</button>
        <!-- 新增配料 -->
    ...
    <div>
      <el-table class="cursor-pointer mt-2" :data="sliceIngredients" highlight-current-row height="440" empty-text="無配料"
...
const addDrink = () => {
  ...
  if (currentDrinkInputId.value <= 0) {
    ElMessage.error('Id不可為負數且需大於0,請重新輸入')
    return
  }
  if (setPriceL.value == true && currentDrinkInputPriceL.value < 0) {
    ElMessage.error('大杯價格不可為負數,請重新輸入')
    return
  }
  if (setPriceBottle.value == true && currentDrinkInputPriceBottle.value < 0) {
...
const editDrink = () => {
  ...
  if (currentEditDrinkInputId.value == currentDrink.value.id && currentEditDrinkInputName.value == currentDrink.value.name && currentEditDrinkInputPriceL.value == currentDrink.value.priceL && currentEditDrinkInputPriceBottle.value == currentDrink.value.priceBottle && currentEditDrinkSelectCustomized.value == currentDrink.value.customized) {
    ...
    if (currentEditDrinkInputId.value <= 0) {
      ElMessage.error('Id不可為負數且需大於0,請重新輸入')
      return
    }
    if (setEditPriceL.value == true && currentEditDrinkInputPriceL.value < 0) {
      ElMessage.error('大杯價格不可為負數,請重新輸入')
      return
    }
    if (setEditPriceBottle.value == true && currentEditDrinkInputPriceBottle.value < 0) {
      ElMessage.error('瓶裝價格不可為負數,請重新輸入')
      return
    }
  }
...
const addDrinkIngredients = () => {
  ...
  if (currentIngredientsInputId.value <= 0) {
    ElMessage.error('Id不可為負數且需大於0,請重新輸入')
    return
  }
  if (currentIngredientsInputPrice.value < 0) {
...
const editDrinkIngredients = () => {
  ...
  if (currentEditIngredientsInputId.value == currentIngredientsDrink.value.id && currentEditIngredientsInputName.value == currentIngredientsDrink.value.name && currentEditIngredientsInputPrice.value == currentIngredientsDrink.value.price) {
    ...
    if (currentEditIngredientsInputId.value <= 0) {
      ElMessage.error('Id不可為負數且需大於0,請重新輸入')
      return
    }
    if (currentEditIngredientsInputPrice.value < 0) {
      ElMessage.error('配料價格不可為負數,請重新輸入')
      return
    }
  }
