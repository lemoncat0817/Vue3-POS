  ...
  <div class="w-full h-full flex flex-col ">
    ...
    <div class="w-full h-full grid grid-cols-5  place-items-center">
      <div @click="changeItem(item)" v-for="item in sliceDrinkMenu" :key="item.id"
        class="2xl:w-28 2xl:h-28 xl:w-24 xl:h-24 lg:w-[72px] lg:h-[72px] bg-red-400 border-solid border-2 border-black rounded-lg  cursor-pointer flex justify-center items-center"
        :class="{ 'bg-yellow-400': item.name === drinkStore.drinkItem.name }">
        <p class="px-2 text-blue-800 2xl:text-xl xl:text-lg lg:text-sm font-bold select-none	">{{ item.name }}</p>
      </div>
    </div>
    <!-- 飲料品項下半部 -->
